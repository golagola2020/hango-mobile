 ...
 */
 if (BOTH_SIDE_SPACE <= distance && distance < (cell*BUTTON_RANGE)+BOTH_SIDE_SPACE)
    for (int k = 0; k < cell ; k++ ){
       if ( ((cell*BUTTON_RANGE)+BOTH_SIDE_SPACE - BUTTON_RANGE*(k+1)) <= distance && distance < ((cell*BUTTON_RANGE)+BOTH_SIDE_SPACE - BUTTON_RANGE*k ) ) sensed_cnt[i][k]++; 
    }
